...
// -----------------------------------------------------------------------------
// MindStreamer Example 06
// Shorted Input Check
//
// What it does:
// - Initializes MindStreamer with the validated operating point
// - Routes all ADS1299 channels to the internal shorted-input mux setting
// - Starts streaming
// - Reads all channels and prints raw values as CSV
//
// Why this is useful:
// - Verifies front-end sanity without electrodes
// - Helps detect abnormal channel behavior
// - Useful for bring-up, regression testing, and production checks
//
// Expected behavior:
// - All channels should stay relatively close to zero
// - All channels should remain reasonably close to each other
// - Large spikes or a single unstable channel may indicate a problem
// -----------------------------------------------------------------------------
...
void setup() {
    ...
    for (uint8_t ch = 1; ch <= NUM_CHANNELS; ++ch) {
        if (!eeg.disableTestSignal(ch)) {
            Serial.print("DISABLE_TEST_FAIL_CH,");
            Serial.print(ch);
            Serial.print(',');
            Serial.println((int)eeg.getLastError());
            while (true) {
        ...
        if (!eeg.setChannelInputMux(ch, ADS1299_MUX_SHORTED)) {
            Serial.print("SET_MUX_SHORTED_FAIL_CH,");
            Serial.print(ch);
            Serial.print(',');
            Serial.println((int)eeg.getLastError());
            while (true) {
